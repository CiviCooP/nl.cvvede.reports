...
{if (!$chartEnabled || !$chartSupported )&& $rows}
    ...
    {/if}
    <table class="report-layout display" style="width: 100%;">
           ...
           {eval var=$sectionHeaderTemplate}
            <tr  class="{cycle values="odd-row,even-row"} {$row.class} crm-report" id="crm-report_{$rowid}">
                {foreach from=$columnHeaders item=header key=field}
                    ...
                    {assign var=fieldImageUrl value=$field|cat:"_imageurl"}
                    {assign var=fieldImageUrlHeight value=$field|cat:"_imageurl_height"}
                    {assign var=fieldImageUrlWidth value=$field|cat:"_imageurl_width"}
                    {if $field == 'manage'}
                        {if ($outputMode == 'pdf' || $outputMode == 'print')}
                            {continue}
                        {/if}
                    ...
                    <td class="crm-report-{$field}{if $header.type eq 1024 OR $header.type eq 1 OR $header.type eq 512} report-contents-right{elseif $row.$field eq 'Subtotal'} report-label{/if}">
                        {if $row.$fieldImageUrl}
                            <a href="{$row.$fieldImageUrl}" class='crm-image-popup'>
                                <img style="vertical-align: top;" src="{$row.$fieldImageUrl}" height={$row.$fieldImageUrlHeight} width={$row.$fieldImageUrlWidth}>
                            </a>&nbsp;
                        {/if}
                    ...
                    <table style="background-color: #f0f0f0;">
                        {foreach from=$row.activities item=activity}
                            <tr>
                                ...
                                <td style="width: 25%">{$activity.subject}</td>
                                <td style="overflow: hidden;">{$activity.details}</td>
                                <td style="width: 10%">{$activity.contacts}</td>
                            </tr>
                        {/foreach}
                    </table>
                </td>
